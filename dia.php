            ...
            <div class="col-lg-8 offset-lg-2">
                <table class="table table-dark">
                    ...
                    <tbody>
                        ...
                        $query = "select hora from eventos where fecha_in='$Fecha' order by hora";
                        $result2 = pg_query($link, $query);
                        while ($line2 = pg_fetch_array($result2, NULL, PGSQL_ASSOC)) {
                        $Hora = $line2["hora"];
                        $hour = substr($Hora,0,2);
                        }
                        pg_close($link);
                        ...
                        if($hour == $x){
                        $link = pg_connect("host=localhost dbname=cc5-agenda user=postgres password=admin");
                        $query = "select E.id_evento,E.titulo,T.nombre from eventos E, tipodeevento T where fecha_in='$Fecha' and T.id_tipo = E.tipo_de_evento order by hora";
